...
app.use(
  cors({
    origin: "*", // Allow all origins or specify your frontend origin: 'http://localhost:3000'
  })
);
var Permission = {
  Allow: "allow",
...
function copyFolder(source, dest) {
  return new Promise((resolve, reject) => {
    ...
    const copyOperations = files.map((file) => {
      return new Promise((innerResolve, innerReject) => {
        ...
        if (fs.lstatSync(curSource).isDirectory()) {
          copyFolder(curSource, curDest).then(innerResolve).catch(innerReject);
        } else {
    ...
    Promise.all(copyOperations).then(resolve).catch(reject);
  });
}
...
function MoveFolder(source, dest) {
  return new Promise((resolve, reject) => {
    ...
    Promise.all(fileOperations).then(resolve).catch(reject);
  });
}
/**
  ...
  return new Promise((resolve, reject) => {
    ...
    fs.stat(filepath, function (err, stats) {
      ...
      if (searchFilterPath) {
        ...
        cwd.filterPath =
          req.body.data.length > 0 && req.body.path != "/"
            ? path
                .normalize(req.body.data[0].filterPath)
                .replace(/^(\.\.[\/\\])+/, "")
                .replace(/\\/g, "/")
      ...
      if (
        ...
        if (req.body.action === "save") {
          ...
          while (fs.existsSync(newName)) {
            ...
            newName = path.join(
              parsedPath.dir,
              `${parsedPath.name}(${fileCount})${parsedPath.ext}`
            );
          }
